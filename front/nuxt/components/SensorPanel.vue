            ...
            <div class="flex justify-between items-center">
              <div>
                <h2 class="text-2xl font-bold text-white">
                  {{ sensor.nombre || `Sensor ${sensor.mac}` }}
                </h2>
...
const confirmDeleteSensor = async () => {
  try {
    ...
    activeSensors.value = activeSensors.value.filter(
      (s) => s.idSensor !== sensorToDelete.value.idSensor
    );
...
const handleUnbanSensor = (sensor) => {
  openConfirmationPopup("Estàs segur que vols desbannejar aquest sensor?", async () => {
    try {
      ...
      bannedSensors.value = bannedSensors.value.filter(
        (s) => s.idSensor !== sensor.idSensor
      );
...
const acceptPendingSensor = (sensor) => {
  openConfirmationPopup("Estàs segur que vols acceptar aquest sensor?", async () => {
    try {
      ...
      pendingSensors.value = pendingSensors.value.filter(
        (s) => s.idSensor !== sensor.idSensor
      );
