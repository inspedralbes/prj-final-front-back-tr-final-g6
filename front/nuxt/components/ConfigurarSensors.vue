<template>
  <div class="min-h-screen bg-slate-800 flex flex-col">
    ...
    <div v-if="showImageUploadModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
...
// Tipos de imágenes con sus nombres requeridos
...
onMounted(async () => {
  loading.value = true;
  try {
    // Aquí iría la llamada a la API para obtener la configuración actual
    // const response = await fetch('/back/principal/sensor/config.json');
    // const data = await response.json();
    // sensorConfig.value = data;
    
    // Por ahora usamos la configuración por defecto
    console.log('Cargando configuración del sensor...');
  } catch (error) {
...
const handleFileChange = (event) => {
  ...
  if (file) {
    // Verificar que el nombre del archivo coincida con el requerido
...
const uploadImage = async () => {
  ...
  try {
    ...
    // const response = await fetch('/back/principal/sensor/images', {
    //   method: 'POST',
    //   body: formData
    // });
    ...
    // Actualizamos la URL en la configuración
...
const saveSensorConfig = async () => {
  try {
    ...
    // await fetch('/back/principal/sensor/config.json', {
    //   method: 'PUT',
    //   headers: {
    //     'Content-Type': 'application/json',
    //   },
    //   body: JSON.stringify(sensorConfig.value)
    // });
