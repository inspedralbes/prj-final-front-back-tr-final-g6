        ...
        <div v-else class="dashboard-content">
            ...
            <div class="dashboard-header">
                ...
                <div class="time-range-info">
                    <div class="time-range-indicator">Last 30 days</div>
...
const lastUpdateTime = computed(() => {
    const latestData = [...temperatureData.value]
        .reverse()
        .find(dataPoint => dataPoint.value !== null && dataPoint.value !== undefined);
    return latestData?.date ? new Date(latestData.date).toLocaleDateString('es-ES') : '--/--/--';
});
...
const fetchInitialData = async () => {
    try {
        ...
        for (let i = 3; i >= 0; i--) {
            ...
            const endDate = new Date(endYear, 5, 24, 23, 59, 59, 999);
            ...
            const average = data?.length
                ? data.reduce((sum, item) => sum + (item.average || 0), 0) / data.length
                : null;
        ...
        temperatureData.value = courses;

        chartData.value.labels = courses.map(c => c.label);
        chartData.value.datasets[0].data = courses.map(c => c.value);
        chartKey.value++;
        loading.value = false;
    } catch (err) {
...
const updateChartData = (data) => {
    chartData.value.labels = data.map(item => item.dateLabel);
...
const handleNewAggregatedData = (data) => {
    ...
    if (sensorCount > 0) {
        ...
        if (avgTemp >= 0 && avgTemp <= 40) {
            ...
            const dayIndex = temperatureData.value.findIndex(
                item => new Date(item.date).toISOString() === todayISO
            );
