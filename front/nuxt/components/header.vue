<template>
  <header class="bg-slate-900 border-b border-slate-700 p-4 shadow-md">
    <div class="max-w-7xl mx-auto flex items-center justify-between">
      <!-- Logo/Link Section -->
      <router-link to="/" class="flex items-center group transition-opacity hover:opacity-90">
        <div
      ...
      <!-- Contenidor dret: Login + Menú -->
      <div class="flex items-center space-x-2">
        <!-- Botó d'inici de sessió -->
        <router-link v-if="userStore.isReady && !userStore.isLoggedIn" to="/login"
          ...
          <span class="hidden sm:inline">Inicia Sessió</span>
        </router-link>
