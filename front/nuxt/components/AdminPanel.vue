                ...
                <div class="p-6 cursor-pointer" @click="toggleDetails(aula)">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex items-center space-x-2">
                            <label :for="'toggle-' + aula.id" class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" :id="'toggle-' + aula.id" v-model="aula.active"
                                    class="sr-only peer">
                                <div
                    ...
                    <div v-if="aula.showDetails" class="mt-4 space-y-4" @click.stop>
                        <template v-if="!aula.editing">
                            ...
                            <p class="text-sm text-gray-700">Planta: {{ aula.Planta !== undefined ? aula.Planta : 'Sense planta' }}</p>
...
onMounted(async () => {
    try {
        aulas.value = (await getAulas()).map(aula => ({ ...aula, active: aula.activa, showDetails: false, editing: false }));
    } catch (error) {
